...
export class CanhotosComponent implements OnInit {
  ...
  async carregar(): Promise<void> {
    ...
    try {
      ...
      this.erro = 'Não foi possível carregar os canhotos.';
    } finally {
  ...
  aplicarFiltros(forcarDetectChanges = true): void {
    ...
    this.entregasFiltradas = this.entregas.filter((entrega) => {
      const possuiCanhoto = this.temCanhoto(entrega);

      const encontrouTipo =
        this.filtroTipo === 'todos' ||
        (this.filtroTipo === 'pendentes' && !possuiCanhoto) ||
        (this.filtroTipo === 'recebidos' && possuiCanhoto);
      ...
      return encontrouTipo && encontrouTexto;
    });
  ...
  temCanhoto(entrega: Entrega): boolean {
    return !!entrega.assinatura_url || !!(entrega.fotos && entrega.fotos.length > 0);
  }
